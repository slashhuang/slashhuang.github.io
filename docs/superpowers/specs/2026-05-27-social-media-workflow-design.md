...
- **内容预览与导出**：在 Web 工作台中实时预览各平台效果，支持复制和导出
...
| 小红书 | 纯文本（300-500字）+ 话题标签 | 自动提取摘要生成短文，带 emoji 和 hashtags |
...
- **XiaohongshuPreview**: 小红书格式预览（纯文本 + emoji + 标签 + 字数统计）
- **ExportPanel**: 导出面板（复制、下载、生成全部）
